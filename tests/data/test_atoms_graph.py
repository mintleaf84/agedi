...
def test_single_graph_cpu_skin_fallback_skips_rebuild(monkeypatch, atoms: "Atoms") -> None:
    """CPU fallback: single graph should skip rebuild when displacement <= skin/2."""
    monkeypatch.setattr(atoms_graph_module, "neighbor_list_needs_rebuild", None)

    graph = AtomsGraph.from_atoms(atoms, skin=0.2)
    edge_index = graph.edge_index.clone()
    # Small nudge well within skin/2 = 0.1 Å
    graph.pos = graph.pos + 0.05

    def fail_make_graph(*args, **kwargs):
        raise AssertionError("make_graph should not be called within skin threshold")

    monkeypatch.setattr(AtomsGraph, "make_graph", staticmethod(fail_make_graph))
    rebuilt = graph.update_graph()
    assert not rebuilt
    assert torch.equal(graph.edge_index, edge_index)


def test_single_graph_cpu_skin_fallback_rebuilds_when_needed(monkeypatch, atoms: "Atoms") -> None:
    """CPU fallback: single graph should rebuild when displacement > skin/2."""
    monkeypatch.setattr(atoms_graph_module, "neighbor_list_needs_rebuild", None)

    graph = AtomsGraph.from_atoms(atoms, skin=0.2)
    # Large displacement well beyond skin/2 = 0.1 Å
    graph.pos = graph.pos + 5.0

    rebuilt = graph.update_graph()
    assert rebuilt


def test_batch_cpu_skin_fallback_skips_rebuild(monkeypatch, atoms: "Atoms") -> None:
    """CPU fallback: batch should skip rebuild when all displacements <= skin/2."""
    monkeypatch.setattr(atoms_graph_module, "batch_naive_neighbor_list", None)
    monkeypatch.setattr(atoms_graph_module, "batch_neighbor_list_needs_rebuild", None)

    g1 = AtomsGraph.from_atoms(atoms, skin=0.2)
    g2 = AtomsGraph.from_atoms(atoms, skin=0.2)
    batch = Batch.from_data_list([g1, g2])
    # Initial rebuild to set reference positions
    batch.update_graph()

    edge_index = batch.edge_index.clone()
    # Small nudge well within skin/2 = 0.1 Å
    batch.pos = batch.pos + 0.05

    def fail_make_graph(*args, **kwargs):
        raise AssertionError("make_graph should not be called within skin threshold")

    monkeypatch.setattr(AtomsGraph, "make_graph", staticmethod(fail_make_graph))
    rebuilt = batch.update_graph()
    assert not rebuilt
    assert torch.equal(batch.edge_index, edge_index)


def test_batch_cpu_skin_fallback_rebuilds_when_needed(monkeypatch, atoms: "Atoms") -> None:
    """CPU fallback: batch should rebuild when any atom displacement > skin/2."""
    monkeypatch.setattr(atoms_graph_module, "batch_naive_neighbor_list", None)
    monkeypatch.setattr(atoms_graph_module, "batch_neighbor_list_needs_rebuild", None)
    ...
    # Large displacement beyond skin/2 = 0.1 Å
    batch.pos = batch.pos + 5.0
